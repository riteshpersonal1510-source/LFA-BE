...
exports.aiPitchGenerator = exports.AIPitchGenerator = void 0;
class AIPitchGenerator {
    ...
    getStrengths(lead) {
        ...
        if (lead.rating && lead.rating >= 4)
            strengths.push('Strong customer ratings and reviews');
        if (lead.reviewsCount && lead.reviewsCount > 50)
            strengths.push('Active customer engagement with many reviews');
        if (lead.trustScore !== undefined && lead.trustScore >= 70)
            strengths.push('High trust signals on website');
        if (lead.socialPresenceScore !== undefined && lead.socialPresenceScore >= 60)
            strengths.push('Decent social media presence');
        if (lead.responsiveScore !== undefined && lead.responsiveScore >= 70)
            strengths.push('Good responsive design');
        if (lead.websiteQualityScore !== undefined && lead.websiteQualityScore >= 70)
            strengths.push('Solid website quality');
        if (lead.aiLeadScore !== undefined && lead.aiLeadScore >= 70)
            strengths.push('High AI-assessed lead potential');
        if (strengths.length === 0)
            strengths.push('Established local business presence');
        return strengths;
    }
    getWeaknesses(lead) {
        const weaknesses = [];
        if (lead.responsiveAudit && !lead.responsiveAudit.mobileFriendly)
            weaknesses.push('Mobile responsiveness needs attention');
        if (lead.responsiveScore !== undefined && lead.responsiveScore < 50)
            weaknesses.push('Poor responsive design score');
        if (lead.seoOpportunity === 'high')
            weaknesses.push('Significant SEO improvement opportunity');
        if (lead.websiteQualityScore !== undefined && lead.websiteQualityScore < 50)
            weaknesses.push('Below average website quality');
        if (lead.websiteFreshness?.status === 'outdated')
            weaknesses.push('Outdated website technology');
        if (lead.socialPresenceScore !== undefined && lead.socialPresenceScore < 40)
            weaknesses.push('Weak social media footprint');
        if (weaknesses.length === 0)
            weaknesses.push('Room for digital presence enhancement');
        return weaknesses;
    }
    getMaturityLabel(lead) {
        if (lead.aiLeadScore !== undefined) {
            if (lead.aiLeadScore >= 70)
                return 'Advanced';
            if (lead.aiLeadScore >= 40)
                return 'Developing';
        }
        if (lead.trustScore !== undefined) {
            if (lead.trustScore >= 70)
                return 'Advanced';
            if (lead.trustScore >= 40)
                return 'Developing';
        }
        return 'Basic';
    }
    getTopIssue(lead) {
        if (lead.responsiveAudit && !lead.responsiveAudit.mobileFriendly)
            return 'mobile optimization needs';
        if (lead.seoOpportunity === 'high')
            return 'SEO improvement potential';
        if (lead.websiteQualityScore !== undefined && lead.websiteQualityScore < 50)
            return 'website quality gaps';
        if (lead.websiteFreshness?.status === 'outdated')
            return 'outdated digital presence';
        return 'digital growth opportunities';
    }
    getServiceFocus(lead) {
        if (lead.responsiveScore !== undefined && lead.responsiveScore < 50)
            return 'responsive redesign';
        if (lead.seoOpportunity === 'high')
            return 'SEO optimization';
        if (lead.websiteQualityScore !== undefined && lead.websiteQualityScore < 50)
            return 'website quality improvement';
        if (lead.socialPresenceScore !== undefined && lead.socialPresenceScore < 40)
    ...
    getRecommendedApproach(lead) {
        ...
        if (lead.responsiveScore !== undefined && lead.responsiveScore < 60)
            services.push('responsive redesign');
        if (lead.seoOpportunity === 'high')
        ...
        if (lead.socialPresenceScore !== undefined && lead.socialPresenceScore < 40)
            services.push('social media setup & management');
        if (lead.websiteFreshness?.status === 'outdated')
...
exports.AIPitchGenerator = AIPitchGenerator;
exports.aiPitchGenerator = new AIPitchGenerator();
//# sourceMappingURL=ai-pitch-generator.js.map