...
class ProposalGenerator {
    constructor(personalization) {
        this.personalization = personalization || personalization_engine_1.personalizationEngine;
    }
    generateSEOProposal(lead) {
        const painPoints = this.personalization.getPrimaryPainPoints(lead);
        const services = this.personalization.getRecommendedServices(lead).filter(s => s.toLowerCase().includes('seo') || s.toLowerCase().includes('optimization') || s.toLowerCase().includes('content'));
        if (services.length === 0)
            services.push('SEO Optimization', 'Local SEO', 'Technical SEO');
    ...
    generateWebsiteRedesignProposal(lead) {
        const painPoints = this.personalization.getPrimaryPainPoints(lead);
    ...
    generateDigitalMarketingProposal(lead) {
        const painPoints = this.personalization.getPrimaryPainPoints(lead);
    ...
    generatePerformanceProposal(lead) {
        const painPoints = this.personalization.getPrimaryPainPoints(lead);
    ...
    generateAll(lead, types) {
        ...
        if (typeSet.size === 0 || typeSet.has('seo'))
            proposals.push(this.generateSEOProposal(lead));
        if (typeSet.size === 0 || typeSet.has('website-redesign'))
            proposals.push(this.generateWebsiteRedesignProposal(lead));
        if (typeSet.size === 0 || typeSet.has('digital-marketing'))
            proposals.push(this.generateDigitalMarketingProposal(lead));
        if (typeSet.size === 0 || typeSet.has('performance'))
            proposals.push(this.generatePerformanceProposal(lead));
        return proposals;
    }
