...
exports.parseProfilePage = parseProfilePage;
const cheerio = __importStar(require("cheerio"));
function parseListingPage(html, existingNames) {
    ...
    for (let i = 0; i < cards.length; i++) {
        ...
        if (existingNames.has(dedupKey) || seen.has(dedupKey))
            continue;
        seen.add(dedupKey);
...
function extractProfileCompanyName($) {
    const selectors = [
...
function extractProfileCategory($) {
    const selectors = [
...
function extractYearOfEstablishment($) {
    const selectors = [
...
function extractCityFromPincode(pincode) {
...
function extractSocialLinks($) {
    ...
    $('a[href]').each((_, el) => {
        ...
        if (lower.includes('linkedin.com') && !social.linkedin)
            social.linkedin = href;
        if (lower.includes('youtube.com') && !social.youtube)
            social.youtube = href;
        if (lower.includes('twitter.com') && !social.twitter)
            social.twitter = href;
...
function extractProfileRating($) {
    const selectors = ['[class*="rating"]', '[class*="star"]', '.rating', '.reviews'];
    for (const sel of selectors) {
        const text = $(sel).first().text().trim();
        const match = text.match(/(\d+\.?\d*)/);
        if (match)
            return { rating: parseFloat(match[1]) };
    }
    return {};
}
...
//# sourceMappingURL=indiamart.parser.js.map