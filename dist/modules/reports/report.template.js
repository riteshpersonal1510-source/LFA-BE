...
function buildReportHtml(summary) {
    ...
    const socialPlatformBadges = summary.socialPlatforms.map(p => `<span class="badge">${escapeHtml(p)}</span>`).join('');
    const websiteTypeMeta = (() => {
        if (isStandalone)
            return { label: 'Standalone Website', cls: 'tag-green' };
        if (summary.websiteType === 'social_only')
            return { label: 'Social Media Only', cls: 'tag-amber' };
        if (summary.websiteType === 'directory_profile')
            return { label: 'Directory Profile', cls: 'tag-amber' };
    ...
    const statusRow = (label, passed, note) => `
    <div class="ch-row">
