...
exports.calculateLeadTrustScore = calculateLeadTrustScore;
exports.calculateDataQuality = calculateDataQuality;
const verify_engine_1 = require("./verify-engine");
function calculateQualityScore(lead) {
    ...
    if (lead.phone) {
        const phoneInfo = (0, verify_engine_1.validatePhone)(lead.phone);
    ...
    if (lead.email) {
        const emailInfo = (0, verify_engine_1.validateEmail)(lead.email);
    ...
    if (lead.category) {
        const normalized = (0, verify_engine_1.normalizeCategory)(lead.category);
...
function calculateLeadTrustScore(lead) {
    let score = 0;
    const weights = {
        phone: 20,
        email: 20,
    ...
    if (lead.phone) {
        const { isValid } = (0, verify_engine_1.validatePhone)(lead.phone);
        score += isValid ? weights.phone : weights.phone * 0.3;
    }
    if (lead.email) {
        const { validationResult } = (0, verify_engine_1.validateEmail)(lead.email);
