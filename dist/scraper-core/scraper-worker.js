...
class ScraperWorker {
    ...
    async execute(session, options) {
        ...
        try {
            ...
            logger_1.logger.error({ err: error.message, keyword, location }, 'ScraperWorker: Execute failed');
            session.fail(error.message);
            throw error;
        }
    }
    async navigateToMaps(page) {
        logger_1.logger.info('ScraperWorker: Navigating to Google Maps...');
        await page.goto('https://www.google.com/maps', {
            waitUntil: 'domcontentloaded',
            timeout: 30000,
        }).catch(() => {
            logger_1.logger.warn('ScraperWorker: Maps navigation timeout, continuing...');
        });
        await page.waitForTimeout(2000);
        logger_1.logger.info('ScraperWorker: Google Maps loaded');
    }
    async searchBusinesses(page, keyword, location) {
        const searchQuery = location
            ? `${keyword} in ${location}`
            : keyword;
        logger_1.logger.info({ searchQuery }, 'ScraperWorker: Searching');
        try {
            ...
            logger_1.logger.warn('ScraperWorker: Primary search failed, trying fallback');
        }
        try {
            ...
            logger_1.logger.warn('ScraperWorker: Fallback search also failed');
        }
    }
    async scrollAndExtract(page, limit, results) {
        logger_1.logger.info({ limit }, 'ScraperWorker: Scrolling and extracting');
        ...
        logger_1.logger.info({ total: results.length }, 'ScraperWorker: Extraction complete');
    }
    async scrollToBottom(page) {
        try {
            ...
            logger_1.logger.warn('ScraperWorker: Scroll failed');
        }
    }
    async extractBusinessesFromPage(page, limit, existingResults) {
        const newBusinesses = [];
        ...
        const businessCount = await page.$$(businessesSelector).then((els) => els.length).catch(() => 0);
        logger_1.logger.info({ count: businessCount }, 'ScraperWorker: Business cards found');
    ...
    async extractSingleBusiness(page, index, selector) {
        try {
    ...
    async extractBusinessDataFromDetails(page) {
        const data = {
        ...
        try {
            ...
            logger_1.logger.warn('ScraperWorker: Error extracting business data');
            return data;
        }
    }
    ...
    async storeLeads(leads) {
        ...
        for (const lead of leads) {
            try {
                ...
                logger_1.logger.info({ company: lead.companyName }, 'ScraperWorker: Lead saved');
            }
            catch (error) {
                logger_1.logger.warn({ err: error.message, company: lead.companyName }, 'ScraperWorker: Store failed');
            }
        }
