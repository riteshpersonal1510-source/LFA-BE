...
exports.businessClassifier = exports.BusinessClassifier = void 0;
const keyword_intelligence_1 = require("./keyword-intelligence");
class BusinessClassifier {
    classify(companyName, category, preferredBusinessType) {
        const lowerName = companyName.toLowerCase().trim();
        const lowerCategory = (category || '').toLowerCase().trim();
        const text = `${lowerName} ${lowerCategory}`;
        const scored = [];
        for (const group of keyword_intelligence_1.keywordIntelligence.getAllGroups()) {
            let score = 0;
            ...
            for (const alias of group.aliases) {
                ...
                if (lowerCategory.includes(alias)) {
                    score += 15;
                    break;
                }
            }
        ...
        if (scored.length > 1 && scored[0].score - scored[1].score < 15) {
            ambiguityWarnings.push(`Ambiguous classification: could be "${scored[0].group.primary}" or "${scored[1].group.primary}"`);
        }
        ...
        if (scored.length === 0) {
            ...
            if (preferredBusinessType && detectedGroup !== preferredBusinessType.toLowerCase().trim()) {
                const preferredGroup = keyword_intelligence_1.keywordIntelligence.getGroup(preferredBusinessType);
                if (preferredGroup) {
                    ...
                    if (preferredScore && preferredScore.score > 0) {
                        ...
                        ambiguityWarnings.push(`Detected as "${detectedGroup}" but searching for "${preferredBusinessType}"`);
                    }
                }
            }
        }
...
exports.businessClassifier = new BusinessClassifier();
//# sourceMappingURL=business-classifier.js.map