...
function preventSSRF(url) {
    ...
    if (blocked.includes(hostname))
        throw new Error('Access to local hosts is not allowed');
    if (hostname.startsWith('192.168.') || hostname.startsWith('10.') || hostname.startsWith('172.'))
        throw new Error('Access to private networks is not allowed');
}
function computeWebsiteHash(html, css, js) {
    return (0, crypto_1.createHash)('md5').update(html.substring(0, 10000) + css.substring(0, 5000) + js.substring(0, 5000)).digest('hex');
}
class WebsiteIntelligenceEngine {
    ...
    async initialize() {
        if (!this.browser) {
            ...
            logger_1.logger.info('Website intelligence engine browser initialized');
        }
    }
    async cleanup() {
        if (this.browser) {
            ...
            this.browser = null;
            logger_1.logger.info('Website intelligence engine browser closed');
        }
    }
    async analyzeWebsite(url, options = {}) {
        ...
        try {
            ...
            logger_1.logger.info(`Starting website intelligence analysis for ${normalizedUrl}`);
            ...
            try {
                ...
                logger_1.logger.info(`Website intelligence completed for ${normalizedUrl} in ${analysisDuration}ms`);
                return {
            ...
            logger_1.logger.error(error instanceof Error ? error : new Error(String(error)), `Website intelligence analysis failed for ${url}:`);
            return this.getDefaultReport();
        }
    }
    async collectPerformanceMetrics(page, url, timeout) {
        let html = '';
        let css = '';
        let js = '';
        try {
            ...
            const allJS = await page.evaluate(() => {
                let jsText = '';
                document.querySelectorAll('script:not([src])').forEach(s => { jsText += s.textContent || ''; });
                return jsText;
            });
            js = allJS;
            ...
            logger_1.logger.warn({ err: String(err), url }, 'Navigation failed during performance collection');
            try {
                html = await page.content();
            }
            catch {
    ...
    async analyzeSecurity(page, url) {
        const parsedUrl = new URL(url);
    ...
    analyzeCategory(html, category) {
        const categoryKey = Object.keys(CATEGORY_FEATURES).find(k => category.toLowerCase().includes(k));
        if (!categoryKey)
            return null;
    ...
    detectIssues(meta, perf, security, seo, ui, content, categorySpecific) {
        const issues = [];
        if (!meta.title) {
            issues.push({
                type: 'missing_title',
    ...
    generateRecommendations(issues, content) {
        const recs = [];
        const criticalIssues = issues.filter(i => i.severity === 'critical');
        const highIssues = issues.filter(i => i.severity === 'high');
        const mediumIssues = issues.filter(i => i.severity === 'medium');
        const byCategory = (cat) => issues.filter(i => i.category === cat);
        ...
        if (mediumIssues.length > 0) {
            const remainingMedium = mediumIssues.filter(i => !recs.some(r => r.description.includes(i.recommendation.substring(0, 30)))).slice(0, 3);
    ...
    calculateScores(meta, perf, security, seo, ui, content, issues) {
        const clamp = (v) => Math.max(0, Math.min(100, v));
        const seoScore = clamp((meta.title ? 10 : 0) +
        ...
        const trustScore = clamp((security.sslValid ? 20 : 0) +
        ...
        const performanceScore = clamp(perf.loadTime === 0 ? 30 :
        ...
        const securityScore = clamp((security.sslValid ? 30 : 0) +
        ...
        const uiScore = clamp((ui.modernDesign ? 15 : 0) +
        ...
        const accessibilityScore = clamp((seo.imageAltTags.totalImages === 0 || (seo.imageAltTags.withAlt / Math.max(seo.imageAltTags.totalImages, 1)) > 0.8 ? 30 : 10) +
            (seo.headingStructure.h1Count > 0 ? 20 : 0) +
            (ui.hasNavigation ? 15 : 0) +
            (seo.imageAltTags.emptyAlt === 0 ? 15 : 5) +
            (ui.viewportConfigured ? 20 : 0));
        const mobileScore = clamp((ui.viewportConfigured ? 25 : 0) +
            (ui.mobileFriendly ? 25 : 0) +
            (ui.responsiveLayout ? 25 : 0) +
            (ui.touchTargetsValid ? 25 : 0));
        const uxScore = clamp((performanceScore * 0.2) +
            (uiScore * 0.3) +
            (mobileScore * 0.2) +
            (accessibilityScore * 0.15) +
            (perf.loadTime < 3000 ? 15 : 5));
        const qualityScore = clamp((seoScore * 0.2) +
            (uiScore * 0.2) +
        ...
        const opportunityMultiplier = Math.min((criticalCount * 3 + highCount * 2 + mediumCount * 1) / 10, 1);
        const businessOpportunityScore = clamp(Math.round((100 - qualityScore) * opportunityMultiplier * 1.5));
        const leadPriorityScore = clamp(Math.round((seoScore * 0.1) +
            (trustScore * 0.15) +
