...
const resume_state_service_1 = require("../../../../services/resume-state.service");
const email_extract_1 = require("../../../../utils/email-extract");
const location_query_builder_1 = require("../../../../utils/location-query-builder");
const navigation_engine_1 = require("../../navigation/navigation-engine");
...
function isEndOfListText(text) {
    return END_OF_LIST_PATTERNS.some(pattern => pattern.test(text));
}
const DEBUG_DIR = path_1.default.resolve(process.cwd(), 'debug-screenshots');
function ensureDebugDir() {
    if (!fs_1.default.existsSync(DEBUG_DIR))
        fs_1.default.mkdirSync(DEBUG_DIR, { recursive: true });
}
class GoogleMapsScraper {
    constructor() {
        this.navigationEngine = new navigation_engine_1.NavigationEngine();
        this.lastProfile = [];
        this.lastActiveOptions = null;
    }
    getProfile() { return this.lastProfile; }
    async scrape(options) {
        const { keyword, location = '', state, city, area, country, businessType, sessionId, semanticKeyword, skipSearchTracking, onStageChange, } = options;
        const reportStage = async (stage, message) => {
            if (skipSearchTracking) {
                await onStageChange?.(stage, message);
            }
            else if (sessionId) {
                search_status_service_1.searchStatus.updateStage(sessionId, stage);
                search_status_service_1.searchStatus.addLog(sessionId, message, 'info');
            }
        };
        ...
        logger_1.logger.info({ searchQuery, searchUrl, country, state, city, area, businessType: businessType || keyword }, 'GoogleMaps: Search query generated');
        this.lastActiveOptions = options;
        const profileLog = [];
        this.lastProfile = profileLog;
        const startTs = Date.now();
        const concurrency = options.detailConcurrency || DEFAULT_WORKERS;
        const profile = (phase, cards, saved, queue) => {
        ...
        try {
            ...
            const acquired = country
                ? await browser_manager_1.browserManager.acquireForCountry('google-maps', country)
                : await browser_manager_1.browserManager.acquire('google-maps');
            ...
            if (!navResult.success && navResult.pageState === navigation_engine_1.PageState.SIGN_IN) {
                await browser_manager_1.browserManager.release(searchPage, 'google-maps');
                searchPage = null;
                ...
                for (let attempt = 1; attempt <= MAX_SIGNIN_RETRIES; attempt++) {
                    ...
                    const fresh = await browser_manager_1.browserManager.acquireForCountry('google-maps-retry', retryCountry);
                    ...
                    if (navResult.success) {
                        logger_1.logger.info({ attempt }, 'GoogleMaps: SIGN_IN retry succeeded via Navigation Engine');
                        signinSuccess = true;
                        break;
                    }
            ...
            const detailPageResults = await browser_manager_1.browserManager.acquireMultiple('gm-detail', concurrency, country);
            detailPages = detailPageResults.map(r => r.page);
            profiling.firstScrollMs = Date.now() - profiling.browserStart;
            const phase2Promise = this.runDetailWorkers(cardQueue, () => queueIndex, (idx) => { queueIndex = idx; }, () => phase1Done, () => phase1Error, context, concurrency, profile, detailPages, () => leadsSaved, (n) => { leadsSaved = n; });
            ...
            context.sessionId && search_status_service_1.searchStatus.updateStage(context.sessionId, 'finalizing');
            context.sessionId && search_status_service_1.searchStatus.addLog(context.sessionId, `Google Maps: ${totalExtracted} found, ${totalStored} saved`, 'info');
            logger_1.logger.info({
                country: country || 'india',
                state: state || '',
                city: city || '',
            ...
            logger_1.logger.error({ err: errMsg, sessionId: context.sessionId, keyword }, 'GoogleMaps: Failed');
            return {
            ...
            await browser_manager_1.browserManager.releaseAll(detailPages, 'gm-detail');
            if (searchPage)
                await browser_manager_1.browserManager.release(searchPage, 'google-maps');
        }
    }
    async collectCards(page, context, cardQueue, profile, allScrapedNames, allScrapedPlaceIds, profiling, allLeads, onSaved, onLeadExtracted, maxResults, resumeSessionId, onProgress, onEndDetected, onSearchCompleted) {
        const resumedPlaceIds = new Set();
        if (resumeSessionId) {
            const state = resume_state_service_1.resumeStateService.load(resumeSessionId);
            if (state) {
                for (const pid of state.processedPlaceIds) {
                    resumedPlaceIds.add(pid);
                    allScrapedPlaceIds.add(pid);
                }
                logger_1.logger.info({ sessionId: resumeSessionId, resumedCount: state.processedCount, savedCount: state.savedCount }, 'GoogleMaps: Resuming from previous state');
            }
        }
        let endRepeats = 0;
        let lastBottomName = '';
        ...
        const saveResumeState = () => {
            ...
            resume_state_service_1.resumeStateService.save(resumeData);
        };
        ...
        for (scrollAttempt = 1; scrollAttempt <= SCROLL_SAFETY_CAP; scrollAttempt++) {
            ...
            if (scrollAttempt > 0 && scrollAttempt % 5 === 0) {
                ...
                if (bi.blocked) {
                    ...
                    logger_1.logger.error({ blockType: bi.type, url: bi.url, title: bi.title }, 'GoogleMaps: Blocked mid-collection');
                    break;
                }
            }
            const metricsBefore = await this.getFeedMetrics(page);
            if (metricsBefore.endOfListVisible || metricsBefore.endOfListText) {
                logger_1.logger.info({ scrollAttempt, found: totalFoundThisSession }, 'GoogleMaps: End of results marker detected');
                await onEndDetected?.();
                break;
            }
            if (maxResults && maxResults > 0 && allScrapedNames.size >= maxResults) {
                logger_1.logger.info({ found: allScrapedNames.size, maxResults }, 'GoogleMaps: Max results limit reached');
                break;
            }
            ...
            for (const card of newCards) {
                ...
                if (allScrapedNames.has(cardKey)) {
                    duplicatesInBatch++;
                    continue;
                }
                if (card.placeId && allScrapedPlaceIds.has(card.placeId)) {
                    duplicatesInBatch++;
                    continue;
                }
                allScrapedNames.add(cardKey);
                if (card.placeId)
                    allScrapedPlaceIds.add(card.placeId);
            ...
            if (actualNew > 0) {
                ...
                if (!profiling.firstCardMs)
                    profiling.firstCardMs = Date.now() - profiling.browserStart;
                profiling.lastCardMs = Date.now() - profiling.browserStart;
                profile('cards-added', cardQueue.length, 0, cardQueue.length);
                const streamResult = await lead_storage_1.leadStorage.storeLeads(streamBatch, this.makeContext(context));
                ...
                if (endRepeats >= MAX_END_REPEATS) {
                    logger_1.logger.info({ endRepeats, found: totalFoundThisSession }, 'GoogleMaps: Same last business repeated — end of results');
                    await onEndDetected?.();
                    break;
                }
            ...
            if (metricsAfter.endOfListVisible || metricsAfter.endOfListText) {
                logger_1.logger.info({ scrollAttempt, found: totalFoundThisSession }, 'GoogleMaps: End of results marker detected after scroll');
                await onEndDetected?.();
                break;
            }
            if (maxResults && maxResults > 0 && allScrapedNames.size >= maxResults)
            ...
            if (consecutiveIdleScrolls >= IDLE_SCROLL_END_THRESHOLD) {
                logger_1.logger.info({ consecutiveIdleScrolls, found: allScrapedNames.size }, `GoogleMaps: ${IDLE_SCROLL_END_THRESHOLD} consecutive idle scrolls — end of results`);
                await onEndDetected?.();
                break;
            }
        ...
        if (scrollAttempt > SCROLL_SAFETY_CAP) {
            logger_1.logger.warn({ found: totalFoundThisSession, cap: SCROLL_SAFETY_CAP }, 'GoogleMaps: Scroll safety cap reached');
        }
        saveResumeState();
        resume_state_service_1.resumeStateService.delete(resumeSessionId || context.sessionId);
        await onSearchCompleted?.();
    }
    async getScrollPercent(page) {
        try {
    ...
    async smartScroll(page) {
        try {
            ...
            await page.evaluate(() => {
                const main = document.querySelector('div[role="main"]');
                if (main) {
                    const step = Math.max(400, Math.min(1200, main.clientHeight * 0.7));
                    main.scrollBy({ top: step, behavior: 'instant' });
                }
                else {
    ...
    async waitForFeedChange(page, before) {
        try {
            const changed = await page.waitForFunction((prev) => {
                const feed = document.querySelector('[role="feed"]');
                const main = document.querySelector('div[role="main"]');
                ...
                return childCount !== prev.childCount
                    ...
                    || isEnd || endOfList;
            }, { childCount: before.childCount, placeLinkCount: before.placeLinkCount }, { timeout: 2500, polling: 150 }).catch(() => null);
            return changed !== null;
        }
        catch {
            return false;
    ...
    isCancelled() {
        try {
            return this.lastActiveOptions?.isCancelled?.() === true;
        }
        catch {
            return false;
        }
    }
    async runDetailWorkers(cardQueue, getIndex, setIndex, isPhase1Done, getPhase1Error, context, concurrency, profile, detailPages, getLeadsSaved, setLeadsSaved) {
        const results = await Promise.allSettled(Array.from({ length: concurrency }, (_, i) => this.detailWorker(cardQueue, getIndex, setIndex, isPhase1Done, getPhase1Error, context, profile, detailPages[i], getLeadsSaved, setLeadsSaved)));
        for (const r of results) {
            if (r.status === 'rejected')
                logger_1.logger.error({ err: r.reason }, 'GoogleMaps: Worker crashed');
        }
    }
    async detailWorker(cardQueue, getIndex, setIndex, isPhase1Done, getPhase1Error, context, profile, page, getLeadsSaved, setLeadsSaved) {
        const buffer = [];
        ...
        const flush = async () => {
            ...
            const enriched = await lead_storage_1.leadStorage.enrichLeads(batch, enrichCtx);
            if (enriched > 0) {
                setLeadsSaved(getLeadsSaved() + enriched);
                profile('enriched', cardQueue.length, getLeadsSaved(), cardQueue.length - getIndex());
                for (const lead of batch) {
                    if (lead.companyName) {
                        logger_1.logger.info({ company: lead.companyName }, 'GoogleMaps: MongoDB saved');
                    }
                }
            }
        };
        try {
            while (true) {
                ...
                try {
                    logger_1.logger.info({ company: card.companyName }, 'GoogleMaps: Business discovered');
                    ...
                    for (const ws of waitStrategies) {
                        try {
                            ...
                            logger_1.logger.debug({ company: card.companyName, strategy: ws, err: navErr instanceof Error ? navErr.message : String(navErr) }, 'GoogleMaps: Nav fallback');
                        }
                    }
                    if (!navSucceeded)
                    ...
                    logger_1.logger.info({ company: card.companyName, url: firstNavUrl, title: firstNavTitle, navMs: Date.now() - navStart }, 'GoogleMaps: Business opened — detail navigated');
                    ...
                    if (bi.blocked) {
                        ...
                        if (bi.type === 'SIGN_IN') {
                            ...
                            for (let retryD = 1; retryD <= MAX_DETAIL_RETRIES; retryD++) {
                                logger_1.logger.warn({ company: card.companyName, attempt: retryD, maxRetries: MAX_DETAIL_RETRIES, type: bi.type }, 'GoogleMaps: Detail page blocked, retrying');
                                try {
                                ...
                                if (!bi.blocked) {
                                    ...
                                    logger_1.logger.info({ company: card.companyName, attempt: retryD }, 'GoogleMaps: Detail page retry succeeded');
                                    break;
                                }
                            ...
                            logger_1.logger.warn({ company: card.companyName, type: bi.type }, 'GoogleMaps: Detail page blocked');
                            throw new Error(`GOOGLE_BLOCKED: ${bi.type}`);
                        }
                    }
                    ...
                    try {
                        ...
                        logger_1.logger.warn({ company: card.companyName }, 'GoogleMaps: Business detail readiness timeout — extracting anyway');
                    }
                    ...
                    logger_1.logger.info({ company: card.companyName }, 'GoogleMaps: Extraction started');
                    ...
                    if (businessStatus)
                        card.businessStatus = businessStatus;
                    if (openingHours)
                        card.workingHours = openingHours;
                    if (plusCode)
                        card.plusCode = plusCode;
                    if (lat !== undefined)
                        card.latitude = lat;
                    if (lng !== undefined)
                        card.longitude = lng;
                    if (secondaryCats && secondaryCats.length > 0)
                        card.secondaryCategories = secondaryCats;
                    if (totalPhotos !== undefined)
                        card.totalPhotos = totalPhotos;
                    if (serviceOpts && serviceOpts.length > 0)
                        card.serviceOptions = serviceOpts;
                    if (ownerClaimed !== undefined)
                        card.ownerClaimed = ownerClaimed;
                    if (addrComponents) {
                        if (addrComponents.streetAddress)
                            card.streetAddress = addrComponents.streetAddress;
                        if (addrComponents.postalCode)
                            card.postalCode = addrComponents.postalCode;
                        if (addrComponents.city)
                            card.city = addrComponents.city;
                        if (addrComponents.state)
                            card.state = addrComponents.state;
                        if (addrComponents.country)
                            card.country = addrComponents.country;
                    }
                    if (!website || !phone || !addr) {
                        ...
                        if (!website) {
                            ...
                            if (retryWebsite)
                                card.website = retryWebsite;
                        }
                        if (!phone) {
                            const retryPhone = await this.extractPhone(page);
                            if (retryPhone)
                                card.phone = retryPhone;
                        }
                        if (!addr) {
    ...
    async captureScreenshot(page, label, info) {
        try {
            ensureDebugDir();
            const url = info?.url || page.url();
            const title = info?.title || await page.title().catch(() => 'unknown');
            const timestamp = Date.now();
            const filepath = path_1.default.join(DEBUG_DIR, `${label}-${timestamp}.png`);
            await page.screenshot({ path: filepath, fullPage: false }).catch(() => { });
            const diagPath = path_1.default.join(DEBUG_DIR, `${label}-${timestamp}.txt`);
            const diagContent = [
                `Label: ${label}`,
                `URL: ${url}`,
                `Title: ${title}`,
            ...
            fs_1.default.writeFileSync(diagPath, diagContent, 'utf-8');
            logger_1.logger.warn({ url, title, type: info?.type, screenshot: filepath }, `GoogleMaps: Screenshot — ${label}`);
            return filepath;
        }
        catch {
            return '';
    ...
    blockedResult(info) {
        ...
        if (isNoResults) {
            logger_1.logger.info({ url: info.url, title: info.title }, 'GoogleMaps: No results found');
        }
        else {
            logger_1.logger.error({ url: info.url, title: info.title, type: info.type }, `GoogleMaps: ${msg}`);
        }
        return {
            success: isNoResults,
    ...
    async warmupPage(page) {
        try {
            await page.goto('https://www.google.com', { waitUntil: 'domcontentloaded', timeout: 15000 });
            await new Promise(r => setTimeout(r, 800 + Math.random() * 1200));
        }
        catch {
            logger_1.logger.debug({}, 'GoogleMaps: Warmup skipped (non-fatal)');
        }
    }
        ...
        try {
            ...
            for (const [pattern, type] of blockedPatterns) {
                if (pattern.test(text))
                    return { blocked: true, type, url, title };
            }
    ...
    async captureDebugEvidence(page, label, info) {
        try {
            ensureDebugDir();
            const url = info.url || page.url();
            const title = info.title || await page.title().catch(() => 'unknown');
            const timestamp = Date.now();
            const screenshotPath = path_1.default.join(DEBUG_DIR, `${label}-${timestamp}.png`);
            const htmlPath = path_1.default.join(DEBUG_DIR, `${label}-${timestamp}.html`);
            const metaPath = path_1.default.join(DEBUG_DIR, `${label}-${timestamp}.json`);
            ...
            fs_1.default.writeFileSync(metaPath, JSON.stringify(diagnosticData, null, 2), 'utf-8');
            logger_1.logger.warn({ url, title, type: info.type, screenshot: screenshotPath, html: htmlPath, metadata: metaPath }, `GoogleMaps: Debug evidence captured — ${label}`);
        }
        catch (err) {
            logger_1.logger.error({ err: err instanceof Error ? err.message : String(err), label }, 'GoogleMaps: Failed to capture debug evidence');
        }
    }
    async capturePageDiagnostics(page) {
        try {
            const diagnostics = browser_manager_1.browserManager.getPageDiagnostics(page);
            if (!diagnostics)
                return {};
    ...
    async waitForBusinessDetailReady(page, timeoutMs) {
        ...
        try {
            ...
            const metrics = await page.evaluate(() => {
                ...
                return {
                    ...
                    scrollHeight: scrollEl ? scrollEl.scrollHeight : document.body.scrollHeight,
                    clientHeight: scrollEl ? scrollEl.clientHeight : window.innerHeight,
                    endOfListVisible,
                };
            });
            return {
    ...
    async extractCards(page, searchRankOffset) {
        try {
            const cards = await page.evaluate((offset) => {
                const leads = [];
                const feed = document.querySelector('[role="feed"]');
                ...
                if (elements.length === 0)
                    elements = Array.from(container.querySelectorAll('a[href*="maps/place/"]'));
                ...
                for (const card of elements) {
                    ...
                    if (name && name.length > 2) {
                        ...
                        if (parts.length >= 2) {
                            ...
                            for (const p of parts.slice(1)) {
                                ...
                                if (rvm)
                                    reviewsCount = parseInt(rvm[1].replace(/,/g, ''), 10) || 0;
                                if (!category && p.length < 60 && !p.match(/^[\d.]+$/) && !p.toLowerCase().includes('star') && !p.toLowerCase().includes('review'))
                                    category = p;
                            }
                        }
                        else {
                    ...
                    if (!name || name.length < 2) {
                        ...
                        if (textParts.length > 0) {
                            ...
                            if (first.length > 1 && first.length < 100 && first !== category)
                                name = first;
                        }
                    }
                    if (!name || name.length < 2) {
                    ...
                    if (!rating) {
                        ...
                        if (ratingEl) {
                            ...
                            if (rvm2)
                                reviewsCount = parseInt(rvm2[1].replace(/,/g, ''), 10) || 0;
                        }
                    }
                    const textContent = card.textContent || '';
                    const segments = textContent.split('·').map(s => s.trim()).filter(Boolean);
                    for (const seg of segments) {
                        if (!category && seg.length < 60 && !seg.match(/^[\d.]+$/) && !seg.toLowerCase().includes('star'))
                            category = seg;
                    }
                    placeId = card.getAttribute('data-place-id') || '';
                    if (!placeId && href) {
                        const m = href.match(/!1s([^!]+)/) || href.match(/place\/([^/]+)/);
                        if (m)
    ...
    parseAddress(fullAddress, contextCity, contextState, contextCountry) {
        if (!fullAddress)
            return { city: contextCity || '', state: contextState || '', area: '', country: contextCountry || '' };
        ...
        if (foundCountry) {
            result.country = foundCountry.split(' ').map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
        }
        ...
        if (!result.city && contextCity)
            result.city = contextCity;
        if (!result.state && contextState)
            result.state = contextState;
        if (!result.country && contextCountry)
            result.country = contextCountry;
        return result;
    }
    async extractWebsite(page) {
        const selectors = [
        ...
        for (const sel of selectors) {
            ...
            if (el) {
                ...
                if (href && !href.includes('google.com')) {
                    const n = this.normalizeWebsite(href);
                    if (n)
                        return n;
                }
            }
        ...
        try {
            const hrefs = await page.evaluate(() => Array.from(document.querySelectorAll('a[href]'))
                ...
                .filter(h => h && !h.includes('google.com') && !h.includes('maps.googleapis') && h.startsWith('http')));
            for (const link of hrefs) {
                const n = this.normalizeWebsite(link);
                if (n)
                    return n;
            }
        }
        ...
        try {
            const jsonLd = await page.evaluate(() => {
                ...
                for (const s of scripts) {
                    try {
                        ...
                        if (obj.url && !obj.url.includes('google.com'))
                            return obj.url;
                        if (obj.sameAs && Array.isArray(obj.sameAs)) {
                            const w = obj.sameAs.find((u) => u.startsWith('http') && !u.includes('google.com'));
                            if (w)
                                return w;
                        }
                    }
            ...
            if (jsonLd) {
                const n = this.normalizeWebsite(jsonLd);
                if (n)
                    return n;
            }
        }
        ...
        try {
            ...
            if (metaUrl) {
                const n = this.normalizeWebsite(metaUrl);
                if (n)
                    return n;
            }
        }
        ...
        try {
            ...
            if (textUrl) {
                const n = this.normalizeWebsite(textUrl);
                if (n)
                    return n;
            }
        }
    ...
    async extractPhone(page) {
        ...
        try {
                ...
                for (const s of scripts) {
                    try {
                        ...
                        if (obj.contactPoint && Array.isArray(obj.contactPoint)) {
                            const cp = obj.contactPoint.find((c) => c.telephone);
                            if (cp)
                                return cp.telephone;
                        }
                    }
            ...
            if (jsonLd) {
                const n = this.normalizePhone(jsonLd);
                if (n)
                    return n;
            }
        }
        ...
        try {
            ...
            if (dataAttrs) {
                const n = this.normalizePhone(dataAttrs);
                if (n)
                    return n;
            }
        }
    ...
    async extractAddress(page) {
            ...
            const addr = await page.evaluate(() => {
                ...
                for (const line of lines) {
                    if (line.match(/\d{4,6}/) || (line.includes(',') && line.length > 15 && line.length < 150)) {
                        if (!line.toLowerCase().includes('phone') && !line.toLowerCase().includes('website'))
                            return line;
                    }
                }
                return '';
        ...
        try {
            const metaAddr = await page.evaluate(() => {
                ...
                if (dc)
                    return dc.getAttribute('content') || '';
                return '';
            });
            if (metaAddr && metaAddr.length > 5)
                return metaAddr;
        }
        catch { }
        try {
    ...
    extractPincode(text) {
        ...
        if (canada)
            return canada[1].replace(/\s/, ' ').toUpperCase();
        const uk = text.match(/\b([A-Za-z]{1,2}\d{1,2}[A-Za-z]?\s?\d[A-Za-z]{2})\b/);
        if (uk)
            return uk[1].toUpperCase();
        const us = text.match(/\b(\d{5})(?:-\d{4})?\b/);
        if (us)
            return us[1];
        return undefined;
    }
    async extractBusinessStatus(page) {
        const selectors = [
    ...
    async extractOpeningHours(page) {
        try {
    ...
    async extractSecondaryCategories(page) {
        try {
            const cats = await page.evaluate(() => {
                ...
                if (button) {
                    ...
                    if (t && t.length < 50 && !categories.includes(t))
                        categories.push(t);
                }
                return categories;
            });
        ...
        try {
            ...
            if (singleCat && singleCat.length > 0 && singleCat.length < 50)
                return [singleCat];
        }
        catch { }
        return undefined;
    }
    async extractTotalPhotos(page) {
        try {
    ...
    async extractServiceOptions(page) {
        try {
    ...
    async extractOwnerClaimed(page) {
        try {
            const claimed = await page.evaluate(() => {
                ...
                if (!panel)
                    return false;
                const text = panel.textContent || '';
                if (text.includes('Claim this business') || text.includes('Own this business?'))
                    return false;
    ...
    async extractAddressComponents(page) {
        try {
            ...
            if (addr) {
                ...
                if (postalCode) {
                    ...
                    if (pIdx > 0)
                        result.state = parts[pIdx - 1];
                    if (pIdx > 1)
                        result.city = parts[pIdx - 2];
                    if (pIdx > 0)
                        result.streetAddress = parts.slice(0, pIdx - (pIdx > 1 ? 2 : 1)).join(', ');
                    if (!result.streetAddress && parts.length > 0)
                ...
                for (const c of knownCountries) {
                    if (lower.includes(c)) {
                        result.country = c.split(' ').map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
                        break;
                    }
                }
                return result;
        ...
        try {
            ...
            if (jsonLd && (jsonLd.streetAddress || jsonLd.city))
                return jsonLd;
        }
        catch { }
        return undefined;
        ...
        try {
            ...
            for (const url of pagesToTry) {
                try {
                    ...
                    const extracted = (0, email_extract_1.extractEmailsFromHtml)(html, 'website');
    ...
    normalizeWebsite(url) {
        if (!url || url.includes('google.com') || url.includes('maps.googleapis'))
            return undefined;
        try {
    ...
    normalizePhone(phone) {
        ...
        if (cleaned.match(/^\+\d{7,15}$/))
            return cleaned;
        if (cleaned.match(/^00\d{9,14}$/))
            return `+${cleaned.substring(2)}`;
        if (cleaned.match(/^\d{11,15}$/))
            return `+${cleaned}`;
        if (cleaned.match(/^0\d{9,14}$/))
