...
exports.leadOpportunityService = exports.LeadOpportunityService = void 0;
class LeadOpportunityService {
    analyze(data) {
        ...
        const issues = [];
        if (websiteMissing)
            issues.push('No website exists');
        if (websiteOutdated)
            issues.push('Website is outdated');
        if (noMobileOptimization)
            issues.push('Not mobile optimized');
        if (missingContactInfo)
            issues.push('Missing contact information');
        if (missingSeo)
            issues.push('Poor SEO');
        if (missingSocialPresence)
        ...
        if (websiteMissing) {
            ...
            if (noSsl)
                recommendedServices.push('SSL Certificate Setup');
            if (noMobileOptimization)
                recommendedServices.push('Mobile Optimization');
            if (missingSeo)
                recommendedServices.push('SEO Services');
        }
        else if (issueCount >= 1) {
            opportunity = 'medium';
            explanation = `Business website needs improvement: ${issues.join(', ')}`;
            if (noMobileOptimization)
                recommendedServices.push('Responsive Design');
            if (missingContactInfo)
                recommendedServices.push('Contact Form Integration');
            if (missingSocialPresence)
                recommendedServices.push('Social Media Integration');
        }
        else {
            opportunity = 'low';
...
exports.LeadOpportunityService = LeadOpportunityService;
exports.leadOpportunityService = new LeadOpportunityService();
//# sourceMappingURL=lead-opportunity.service.js.map