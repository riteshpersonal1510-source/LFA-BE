        ...
        while (true) {
            ...
            try {
                ...
                if (msg.includes('aborted') || msg.includes('fetch failed') || msg.includes('ECONNREFUSED')) {
                    logger_1.logger.warn({ err: msg, jobId, sessionId }, '[PYTHON_SCRAPER] Transient network error during poll, retrying...');
                    search_status_service_1.searchStatus.addLog(sessionId, `[PYTHON_SCRAPER] Transient network error: ${msg}. Retrying...`, 'warn');
                    await new Promise((resolve) => setTimeout(resolve, pollIntervalMs));
                    continue;
                }
