...
function extractMergeKeys(lead: LeadData): MergeKey[] {
  const keys: MergeKey[] = [];
...
function mergeField<T>(existing: T | undefined, incoming: T | undefined, existingPriority: number, incomingPriority: number): T | undefined {
...
export class MergeEngine {
  private readonly KEY_SIMILARITY_THRESHOLD = 0.6;
  ...
  private cluster(leads: LeadData[]): LeadData[][] {
    const groups: LeadData[][] = [];
    const assigned = new Set<number>();

    for (let i = 0; i < leads.length; i++) {
      if (assigned.has(i)) continue;

      const group: LeadData[] = [leads[i]];
      assigned.add(i);
  ...
  private keysMatch(a: MergeKey[], b: MergeKey[]): boolean {
    for (const ka of a) {
      for (const kb of b) {
        if (ka.type !== kb.type) continue;

        if (ka.type === 'placeId' || ka.type === 'mapsUrl') {
          if (ka.value === kb.value) return true;
        }
        ...
        if (ka.type === 'businessName') {
          if (ka.value === kb.value) return true;
          if (this.stringSimilarity(ka.value, kb.value) > this.KEY_SIMILARITY_THRESHOLD) return true;
        }

        if (ka.type === 'phone') {
          const aPhone = ka.value.replace(/^91/, '');
          const bPhone = kb.value.replace(/^91/, '');
          if (aPhone === bPhone || aPhone.endsWith(bPhone) || bPhone.endsWith(aPhone)) return true;
        }
  ...
  private mergeGroup(group: LeadData[]): MergedLead {
    ...
    for (const lead of sorted.slice(1)) {
      ...
      if (merged.rating === undefined && lead.rating !== undefined) merged.rating = lead.rating;
      if (merged.reviewsCount === undefined && lead.reviewsCount !== undefined) merged.reviewsCount = lead.reviewsCount;

      if (!merged.placeId && lead.placeId) merged.placeId = lead.placeId;
    }
  ...
  private calculateScore(lead: MergedLead): number {
    let score = 0;
    ...
    if (lead.mergedSources && lead.mergedSources.length > 1) score += 10;
    if (lead.mergedSources && lead.mergedSources.length > 2) score += 5;
