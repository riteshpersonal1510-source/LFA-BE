...
export class SeoAuditService {
  auditFromHtml(html: string): SeoAuditResult {
    ...
    else if (!titleOk) issues.push(`Title length ${titleLength} chars (recommended 30-60)`);
    ...
    else if (!metaDescriptionOk) issues.push(`Meta description length ${metaDescriptionLength} chars (recommended 120-160)`);
    ...
    if (!h1Present) issues.push('Missing H1 heading');
    else if (h1Count > 1) issues.push(`Multiple H1 tags (${h1Count})`);

    const robotsMeta = this.extractMetaRobots(html);
    if (robotsMeta.includes('noindex')) issues.push('Page blocked from indexing (noindex)');
    if (robotsMeta.includes('nofollow')) issues.push('Links blocked from crawling (nofollow)');
    ...
    if (!ogPresent) issues.push('Missing Open Graph tags');

    const twitterCard = this.extractMetaName(html, 'twitter:card');
    const twitterPresent = !!twitterCard;

    if (!twitterPresent) issues.push('Missing Twitter Card tags');
    ...
    let score = 50;
    if (titleOk) score += 10;
    if (metaDescriptionOk) score += 10;
    if (h1Present && h1Count === 1) score += 10;
    if (!robotsMeta.includes('noindex')) score += 5;
    if (canonicalPresent) score += 5;
    if (ogPresent) score += 5;
    if (twitterPresent) score += 5;
    if (jsonLdPresent) score += 5;
    if (faviconPresent) score += 5;
  ...
  private extract(html: string, pattern: RegExp): string {
    const m = html.match(pattern);
    return m ? m[1].trim() : '';
  }

  private extractAll(html: string, pattern: RegExp): string[] {
  ...
  private extractMetaRobots(html: string): string {
  ...
  private extractSchemaOrgTypes(html: string): string[] {
  ...
  private hasFavicon(html: string): boolean {
