...
const SOCIAL_PATTERNS: Array<{ key: string; patterns: string[] }> = [
...
export class ContactDiscoveryService {
  discoverFromPages(pages: CrawledPageForContacts[]): ContactDiscoveryResult {
    const socialLinks: ContactDiscoveryResult['socialLinks'] = {
    ...
    for (const link of allLinks) {
      ...
      for (const social of SOCIAL_PATTERNS) {
        if (socialLinks[social.key as keyof typeof socialLinks]) continue;
        for (const pattern of social.patterns) {
          if (lower.includes(pattern)) {
            (socialLinks as Record<string, string | string[]>)[social.key] = link;
            break;
          }
        }
      }
    ...
    for (const indicator of CONTACT_FORM_INDICATORS) {
      ...
      if (m) {
        ...
        if (actionMatch) contactFormAction = actionMatch[1];
        break;
      }
    }
