...
export interface LeadData {
  ...
  websiteQuality?: { score?: number; issues?: string[] };
}
...
export class LeadScoringService {
  calculate(data: LeadData): LeadScoreResult {
    ...
    if (data.responsiveScore !== undefined) {
      ...
      if (data.responsiveScore >= 70) reasoning.push('Good responsive design');
      else if (data.responsiveScore >= 40) reasoning.push('Average responsive design');
      else reasoning.push('Poor responsive design');
    } else {
    ...
    if (data.seoScore !== undefined) {
      ...
      if (data.seoScore >= 70) reasoning.push('Good SEO');
      else if (data.seoScore >= 40) reasoning.push('Average SEO — improvement needed');
      else reasoning.push('Poor SEO — significant opportunity');
    }
    ...
    if (data.socialLinks) {
      ...
      if (platformCount > 0) reasoning.push(`${platformCount} social platforms detected`);
      else reasoning.push('No social media presence');
    }

    let businessStrength = 0;
    if (data.rating) {
      if (data.rating >= 4.5) { businessStrength += 8; reasoning.push('Excellent rating'); }
      else if (data.rating >= 4.0) { businessStrength += 5; reasoning.push('Good rating'); }
      else { businessStrength += 2; }
    }
    if (data.reviewsCount && data.reviewsCount > 50) { businessStrength += 5; reasoning.push('Strong review count'); }
    else if (data.reviewsCount && data.reviewsCount > 10) { businessStrength += 2; }
