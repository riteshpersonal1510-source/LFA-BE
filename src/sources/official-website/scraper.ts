...
export class OfficialWebsiteSource extends BaseSource {
  ...
  async scrape(options: SourceOptions): Promise<ScrapingResult> {
    ...
    logger.info({ keyword, location, limit }, 'OfficialWebsiteSource: Starting scrape');
    ...
    const poolResource = await browserPool.acquire('official-website');
    ...
    try {
      ...
      for (const site of batch) {
        try {
          ...
          logger.debug({ url: site.url, error: msg }, 'OfficialWebsiteSource: Failed to scrape site');
        }
      ...
      logger.error({ err: message, keyword, location }, 'OfficialWebsiteSource: Scrape failed');
      ...
      await browserPool.release(poolResource.page, 'official-website');
    }
  }

  private async extractSearchLinks(page: any, maxLinks: number): Promise<Array<{ title: string; url: string; snippet: string }>> {
    ...
    try {
      ...
      logger.warn({ err: error instanceof Error ? error.message : 'Unknown' }, 'OfficialWebsiteSource: extractSearchLinks failed');
    }

    return links;
  }

  private filterBusinessWebsites(links: Array<{ title: string; url: string; snippet: string }>): Array<{ title: string; url: string; snippet: string }> {
  ...
  private async scrapeWebsite(page: any, url: string, siteTitle: string, keyword: string): Promise<LeadData | null> {
    try {
      ...
      if (!contactEmail || !contactPhone || !contactAddress) {
        for (const contactUrl of contactUrls) {
          try {
            ...
            if (!contactEmail) contactEmail = await this.extractEmail(page);
            if (!contactPhone) contactPhone = await this.extractPhone(page);
            if (!contactAddress) contactAddress = await this.extractAddress(page);
      ...
      logger.debug({ url, err: error instanceof Error ? error.message : 'Unknown' }, 'OfficialWebsiteSource: scrapeWebsite failed');
      return null;
    }
  }

  private extractCompanyName(title: string, siteTitle: string, _keyword: string): string | null {
  ...
  private async extractEmail(page: any): Promise<string | null> {
    try {
      ...
      if (emails) {
        const valid = emails.filter((e: string) => {
  ...
  private async extractPhone(page: any): Promise<string | null> {
    try {
      const body = await page.textContent('body').catch(() => '');
      ...
      if (phones) {
        const valid = phones.filter((p: string) => {
  ...
  private async extractAddress(page: any): Promise<string | null> {
    try {
