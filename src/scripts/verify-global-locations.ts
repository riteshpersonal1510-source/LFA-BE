/**
 * Verification script for global location query generation.
 * Run: npx ts-node src/scripts/verify-global-locations.ts
 */

import { buildLocationString, buildMapsSearchQuery } from '../utils/location-query-builder';
...
function run(): void {
  ...
  for (const tc of TEST_CASES) {
    ...
    const locationString = buildLocationString(parts);
    const { searchQuery } = buildMapsSearchQuery(tc.keyword, parts);
  ...
  // Area-specific format test
  const withArea = buildMapsSearchQuery('Gym', {
    area: 'Satellite',
    city: 'Ahmedabad',
    state: 'Gujarat',
    country: 'India',
  ...
  const withoutArea = buildMapsSearchQuery('Dentist', {
    city: 'London',
